		...
		<ul class="flex">
			...
			<c:if test="${rq.getLoginedMemberId() != 0 }">
				<c:if test="${rq.getLoginedMember().getAuthLevel() == 7 }">
					<li class="hover:underline"><a class="h-full px-3 flex items-center" href="/usr/member/memberlist"><span>회원명단</span></a></li>
				</c:if>
			</c:if>
			<c:if test="${rq.getLoginedMemberId() != 0 }">
				<li class="hover:underline"><a class="h-full px-3 flex items-center" href="/usr/member/profile"><span>마이페이지</span></a></li>
